    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('stakeholder-communications.update', ['stakeholder' => $stakeholder, 'communication' => $communication]) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <input type="hidden" name="meeting_date" id="meeting_date_hidden" value="{{ old('meeting_date', $communication->meeting_date->format('Y-m-d')) }}">
                                <input type="hidden" name="meeting_time" id="meeting_time_hidden" value="{{ old('meeting_time', $communication->meeting_time) }}">

                                <div class="form-group mb-3">
                                    <label for="meeting_type">Meeting Type <span class="text-danger">*</span></label>
                                    <select name="meeting_type" id="meeting_type" class="form-control @error('meeting_type') is-invalid @enderror" required>
                                        <option value="">Select Type</option>
                                        <option value="in-person" {{ old('meeting_type', $communication->meeting_type) == 'in-person' ? 'selected' : '' }}>In Person</option>
                                        <option value="video" {{ old('meeting_type', $communication->meeting_type) == 'video' ? 'selected' : '' }}>Video</option>
                                        <option value="phone" {{ old('meeting_type', $communication->meeting_type) == 'phone' ? 'selected' : '' }}>Phone</option>
                                        <option value="email" {{ old('meeting_type', $communication->meeting_type) == 'email' ? 'selected' : '' }}>Email</option>
                                    </select>
                                    @error('meeting_type')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <textarea name="attendees" id="attendees" rows="2" 
                                        class="form-control @error('attendees') is-invalid @enderror" required>{{ old('attendees', $communication->attendees) }}</textarea>
                                    @error('attendees')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <textarea name="discussion_points" id="discussion_points" rows="3" 
                                        class="form-control @error('discussion_points') is-invalid @enderror" required>{{ old('discussion_points', $communication->discussion_points) }}</textarea>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <textarea name="action_items" id="action_items" rows="2" 
                                        class="form-control @error('action_items') is-invalid @enderror">{{ old('action_items', $communication->action_items) }}</textarea>
                                    @error('action_items')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <textarea name="follow_up_notes" id="follow_up_notes" rows="3" 
                                        class="form-control @error('follow_up_notes') is-invalid @enderror">{{ old('follow_up_notes', $communication->follow_up_notes) }}</textarea>
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" name="follow_up_date" id="follow_up_date" 
                                        ...
                                        value="{{ old('follow_up_date', $communication->follow_up_date?->format('Y-m-d')) }}">
                        ...
                        <div class="d-flex justify-content-between mt-3">
                            ...
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save"></i> Update Communication
                            </button>
                        </div>
                    </form>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const meetingDatepicker = flatpickr("#meeting_datetime", {
        ...
        time_24hr: false,
        defaultDate: "{{ $communication->meeting_date->format('Y-m-d') }} {{ $communication->meeting_time }}",
        noCalendar: false,
        disableMobile: true,
        minuteIncrement: 15,
        allowInput: true,
